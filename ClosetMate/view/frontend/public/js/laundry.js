...
document.addEventListener("DOMContentLoaded", () => {
  console.log("Laundry Manager page loaded ✅");
  ...
  function prepareUnassignedItems() {
    laundryGrid.querySelectorAll(".laundry-item").forEach(item => {
      ...
      if (item.dataset.status === "unassigned") {
        if (!item.querySelector(".suggestion")) {
          ...
          msg.textContent =
            "This laundry item is unassigned. Please choose Wash, Dry, or Fold.";
  ...
  addBtn.addEventListener("click", () => {
    const hamperItems = [...document.querySelectorAll(".hamper-item img")];
    if (hamperItems.length === 0) {
      alert("No hamper items found.");
      return;
    }

    const hamperGallery = hamperItems
      .map(img => `
        <div class="hamper-pick" data-src="${img.src}" data-name="${img.alt}">
          <img src="${img.src}" alt="${img.alt}">
          <div class="decision-buttons">
            <label><input type="radio" name="laundry-${img.alt}" value="wash"> Wash</label>
            <label><input type="radio" name="laundry-${img.alt}" value="dry"> Dry</label>
            <label><input type="radio" name="laundry-${img.alt}" value="fold"> Fold</label>
            <label><input type="radio" name="laundry-${img.alt}" value="unassigned"> Leave Unassigned</label>
          </div>
        </div>
      `).join("");

    openModal(`
      <h2>Select Laundry and Assign Task</h2>
      <div class="hamper-gallery">${hamperGallery}</div>
  ...
  document.addEventListener("click", e => {
    if (e.target.id === "confirm-add") {
      const picks = [...document.querySelectorAll(".hamper-pick")];
      let added = 0;
      ...
      picks.forEach(pick => {
        ...
        if (choice) {
          ...
          newItem.classList.add("laundry-item");
          ...
          `;
          laundryGrid.appendChild(newItem);
      ...
      if (added === 0) {
        alert("Pick at least one laundry item.");
        return;
      }
  ...
  document.addEventListener("change", e => {
    if (e.target.classList.contains("item-checkbox") && editMode) {
      const item = e.target.closest(".laundry-item");
      if (e.target.checked) {
        addDecisionButtons(item);
      } else {
        const btns = item.querySelector(".decision-buttons");
  ...
  deleteBtn.addEventListener("click", () => {
    if (!deleteMode) {
      ...
      const selected = laundryGrid.querySelectorAll(".item-checkbox:checked");
      if (selected.length === 0) {
        alert("Select items to delete.");
        return;
      }
      if (confirm(`Delete ${selected.length} item(s)?`)) {
        selected.forEach(cb => cb.closest(".laundry-item").remove());
      }
      deleteMode = false;
      deleteBtn.textContent = "Delete";
      laundryGrid.querySelectorAll(".item-checkbox").forEach(cb => cb.remove());
    }
  });
  ...
  function addDecisionButtons(item) {
    ...
    btns.innerHTML = `
      <button class="wash" data-status="wash">Wash</button>
      <button class="dry" data-status="dry">Dry</button>
      <button class="fold" data-status="fold">Fold</button>
    `;
  ...
  document.addEventListener("click", e => {
    if (e.target.closest(".decision-buttons button")) {
      const btn = e.target;
      const item = btn.closest(".laundry-item");
