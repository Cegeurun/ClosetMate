document.addEventListener("DOMContentLoaded", async () => {
  ...
  // mainmenu.js
  // =========================
  // Initialize user data (Custer)
  const userId = Number(localStorage.getItem("userId")); // <-- fixed quotes
  alert(`now trying to fetch`);
  const res = await fetch(`/user/${userId}`); // call backend
  const userData = await res.json();    // parse JSON
